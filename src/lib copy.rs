...
impl PyValue {
    pub fn from_py_any(obj: &PyAny) -> PyResult<Self> {
        Ok(Self { inner: core_from_py(obj)? })
    }
}

pub fn core_from_py(obj: &PyAny) -> PyResult<CoreValue> {
...
pub fn core_to_py<'py>(v: &CoreValue, py: Python<'py>) -> &'py PyAny {
    match v {
        CoreValue::Null     => py.None().into_ref(py),
        CoreValue::Bool(b)  => b.to_object(py).into_ref(py),
        CoreValue::Int(i)   => i.to_object(py).into_ref(py),
        CoreValue::Float(f) => f.to_object(py).into_ref(py),
        CoreValue::Str(s)   => s.to_object(py).into_ref(py),
        CoreValue::List(items) => {
            let v: Vec<&PyAny> = items.iter().map(|i| core_to_py(i, py)).collect();
            PyList::new(py, v)
        }
        ...
        CoreValue::Iter(items, idx) => {
            let v: Vec<&PyAny> = items[*idx..].iter().map(|i| core_to_py(i, py)).collect();
            PyList::new(py, v)
        }
    }
}
...
impl PyValue {
    ...
    fn new(value: &PyAny, _type_name: Option<String>) -> PyResult<Self> {
        Ok(Self { inner: core_from_py(value)? })
    }
    #[classmethod]
    fn from_python(_cls: &PyType, obj: &PyAny) -> PyResult<Self> {
        Ok(Self { inner: core_from_py(obj)? })
    }
    fn to_python(&self, py: Python<'_>) -> PyObject {
        core_to_py(&self.inner, py).to_object(py)
    }
...
impl NativeRegistry {
    pub fn get_function(&self, func_id: u32) -> Option<PyObject> {
        self.functions.lock().unwrap().get(&func_id).cloned()
    }
...
impl VmProxy {
    ...
    fn __getattr__(&self, name: &str) -> PyResult<PyObject> {
        self.registry.attrs.lock().unwrap().get(name).cloned()
...
pub struct VM {
    ...
    // Keep registry accessible for CALL_EXT handlers
...
impl VM {
    ...
    fn load(&mut self, bytecode: &PyBytes) -> PyResult<()> {
    ...
    /// Register an ISA before calling execute().
    /// Usage: vm.register_isa(AvapISA())
    /// The ISA object must expose _get_arc_ptr() -> u64 returning
    /// an Arc<dyn ISAProvider> raw pointer (refcount already incremented).
    fn register_isa(&mut self, isa_obj: PyObject, py: Python<'_>) -> PyResult<()> {
        // Call _get_arc_ptr() on the ISA Python object
        let ptr_val = isa_obj
            .call_method0(py, "_get_arc_ptr")
            .map_err(|_| PyRuntimeError::new_err(
                "ISA object must implement _get_arc_ptr() -> u64.\n                 Use AvapISA() from the avap-isa package."
            ))?;
    ...
    fn execute(
        ...
        if let Some(r) = registry.clone() {
            ...
            // Build a PyDict { func_id(int) -> callable } and store its pointer
            // in VMState so avap-isa CALL_EXT can look up functions without
            // knowing about NativeRegistry's internal structure.
        ...
        // Pass Python GIL pointer to handlers that need it (CALL_EXT)
        ...
        // Clean up registry pointer (was Py<PyDict>)
    ...
    fn globals(&self, py: Python<'_>) -> PyObject {
        ...
        for (k, v) in &self.state.globals {
            let _ = d.set_item(k, Py::new(py, PyValue { inner: v.clone() }).unwrap().to_object(py));
        }
        d.to_object(py)
    }
    #[setter]
    fn set_globals(&mut self, py: Python<'_>, dict: &PyDict) -> PyResult<()> {
...
#[pymodule]
fn platon(py: Python<'_>, m: &PyModule) -> PyResult<()> {
    m.add_class::<VM>()?;
